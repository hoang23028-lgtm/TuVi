/**
 * Chuyển đổi Dương lịch sang Âm lịch
 * Sử dụng thuật toán và bảng dữ liệu chính xác
 */

// Dữ liệu âm lịch từ 1900-2100
// Mỗi năm được encode thành 1 số:
// - Bit 0-11: Tháng lớn/nhỏ (1=30 ngày, 0=29 ngày)
// - Bit 12-15: Tháng nhuận (0=không nhuận)
// - Bit 16-19: Ngày đầu tháng nhuận lớn/nhỏ
const LUNAR_DATA = [
  0x04bd8, 0x04ae0, 0x0a570, 0x054d5, 0x0d260, 0x0d950, 0x16554, 0x056a0, 0x09ad0, 0x055d2, // 1900-1909
  0x04ae0, 0x0a5b6, 0x0a4d0, 0x0d250, 0x1d255, 0x0b540, 0x0d6a0, 0x0ada2, 0x095b0, 0x14977, // 1910-1919
  0x04970, 0x0a4b0, 0x0b4b5, 0x06a50, 0x06d40, 0x1ab54, 0x02b60, 0x09570, 0x052f2, 0x04970, // 1920-1929
  0x06566, 0x0d4a0, 0x0ea50, 0x06e95, 0x05ad0, 0x02b60, 0x186e3, 0x092e0, 0x1c8d7, 0x0c950, // 1930-1939
  0x0d4a0, 0x1d8a6, 0x0b550, 0x056a0, 0x1a5b4, 0x025d0, 0x092d0, 0x0d2b2, 0x0a950, 0x0b557, // 1940-1949
  0x06ca0, 0x0b550, 0x15355, 0x04da0, 0x0a5b0, 0x14573, 0x052b0, 0x0a9a8, 0x0e950, 0x06aa0, // 1950-1959
  0x0aea6, 0x0ab50, 0x04b60, 0x0aae4, 0x0a570, 0x05260, 0x0f263, 0x0d950, 0x05b57, 0x056a0, // 1960-1969
  0x096d0, 0x04dd5, 0x04ad0, 0x0a4d0, 0x0d4d4, 0x0d250, 0x0d558, 0x0b540, 0x0b6a0, 0x195a6, // 1970-1979
  0x095b0, 0x049b0, 0x0a974, 0x0a4b0, 0x0b27a, 0x06a50, 0x06d40, 0x0af46, 0x0ab60, 0x09570, // 1980-1989
  0x04af5, 0x04970, 0x064b0, 0x074a3, 0x0ea50, 0x06b58, 0x05ac0, 0x0ab60, 0x096d5, 0x092e0, // 1990-1999
  0x0c960, 0x0d954, 0x0d4a0, 0x0da50, 0x07552, 0x056a0, 0x0abb7, 0x025d0, 0x092d0, 0x0cab5, // 2000-2009
  0x0a950, 0x0b4a0, 0x0baa4, 0x0ad50, 0x055d9, 0x04ba0, 0x0a5b0, 0x15176, 0x052b0, 0x0a930, // 2010-2019
  0x07954, 0x06aa0, 0x0ad50, 0x05b52, 0x04b60, 0x0a6e6, 0x0a4e0, 0x0d260, 0x0ea65, 0x0d530, // 2020-2029
  0x05aa0, 0x076a3, 0x096d0, 0x04afb, 0x04ad0, 0x0a4d0, 0x1d0b6, 0x0d250, 0x0d520, 0x0dd45, // 2030-2039
  0x0b5a0, 0x056d0, 0x055b2, 0x049b0, 0x0a577, 0x0a4b0, 0x0aa50, 0x1b255, 0x06d20, 0x0ada0, // 2040-2049
  0x14b63, 0x09370, 0x049f8, 0x04970, 0x064b0, 0x168a6, 0x0ea50, 0x06b20, 0x1a6c4, 0x0aae0, // 2050-2059
  0x092e0, 0x0d2e3, 0x0c960, 0x0d557, 0x0d4a0, 0x0da50, 0x05d55, 0x056a0, 0x0a6d0, 0x055d4, // 2060-2069
  0x052d0, 0x0a9b8, 0x0a950, 0x0b4a0, 0x0b6a6, 0x0ad50, 0x055a0, 0x0aba4, 0x0a5b0, 0x052b0, // 2070-2079
  0x0b273, 0x06930, 0x07337, 0x06aa0, 0x0ad50, 0x14b55, 0x04b60, 0x0a570, 0x054e4, 0x0d160, // 2080-2089
  0x0e968, 0x0d520, 0x0daa0, 0x16aa6, 0x056d0, 0x04ae0, 0x0a9d4, 0x0a4d0, 0x0d150, 0x0f252, // 2090-2099
  0x0d520, // 2100
];

// Lấy tháng nhuận của năm âm lịch
function leapMonth(year: number): number {
  const idx = year - 1900;
  if (idx < 0 || idx >= LUNAR_DATA.length) return 0;
  return LUNAR_DATA[idx] & 0xf;
}

// Lấy số ngày trong tháng nhuận
function leapDays(year: number): number {
  if (leapMonth(year) === 0) return 0;
  const idx = year - 1900;
  if (idx < 0 || idx >= LUNAR_DATA.length) return 0;
  return (LUNAR_DATA[idx] & 0x10000) ? 30 : 29;
}

// Lấy số ngày trong năm âm lịch
function yearDays(year: number): number {
  const idx = year - 1900;
  if (idx < 0 || idx >= LUNAR_DATA.length) return 365;
  let sum = 348; // 12 * 29
  let data = LUNAR_DATA[idx];
  for (let i = 0x8000; i > 0x8; i >>= 1) {
    sum += (data & i) ? 1 : 0;
  }
  return sum + leapDays(year);
}

// Lấy số ngày trong tháng âm lịch
function monthDays(year: number, month: number): number {
  const idx = year - 1900;
  if (idx < 0 || idx >= LUNAR_DATA.length) return 29;
  return (LUNAR_DATA[idx] & (0x10000 >> month)) ? 30 : 29;
}

export interface LunarDate {
  day: number;
  month: number;
  year: number;
  isLeapMonth: boolean;
  lunarYearCanChi: string;
}

/**
 * Chuyển dương lịch sang âm lịch
 */
export function solarToLunar(solarYear: number, solarMonth: number, solarDay: number): LunarDate {
  // Ngày cơ sở: 31/1/1900 = Mùng 1 tháng Giêng năm Canh Tý
  const baseDate = new Date(1900, 0, 31); // 31 Jan 1900
  const targetDate = new Date(solarYear, solarMonth - 1, solarDay);
  
  let offset = Math.floor((targetDate.getTime() - baseDate.getTime()) / 86400000);
  
  // Tìm năm âm lịch
  let lunarYear = 1900;
  let temp = 0;
  for (let i = 1900; i < 2101 && offset > 0; i++) {
    temp = yearDays(i);
    offset -= temp;
    lunarYear++;
  }
  if (offset < 0) {
    offset += temp;
    lunarYear--;
  }
  
  // Tìm tháng âm lịch
  let lunarMonth = 1;
  let isLeapMonth = false;
  const leap = leapMonth(lunarYear);
  let isLeap = false;
  
  for (let i = 1; i < 13 && offset > 0; i++) {
    // Tháng nhuận
    if (leap > 0 && i === (leap + 1) && !isLeap) {
      --i;
      isLeap = true;
      temp = leapDays(lunarYear);
    } else {
      temp = monthDays(lunarYear, i);
    }
    
    // Nếu đã qua tháng nhuận
    if (isLeap && i === (leap + 1)) {
      isLeap = false;
    }
    
    offset -= temp;
    if (!isLeap) {
      lunarMonth++;
    }
  }
  
  if (offset < 0) {
    offset += temp;
    lunarMonth--;
  }
  
  // Kiểm tra xem tháng hiện tại có phải tháng nhuận không
  if (leap > 0 && lunarMonth === leap + 1) {
    if (isLeap) {
      isLeapMonth = true;
    }
  }
  
  // Nếu lunarMonth > 12, điều chỉnh
  if (lunarMonth > 12) {
    lunarMonth -= 12;
    lunarYear++;
  }
  
  const lunarDay = offset + 1;
  
  // Tính Can Chi năm
  const canIndex = (lunarYear - 4) % 10;
  const chiIndex = (lunarYear - 4) % 12;
  const CAN = ['Giáp', 'Ất', 'Bính', 'Đinh', 'Mậu', 'Kỷ', 'Canh', 'Tân', 'Nhâm', 'Quý'];
  const CHI = ['Tý', 'Sửu', 'Dần', 'Mão', 'Thìn', 'Tỵ', 'Ngọ', 'Mùi', 'Thân', 'Dậu', 'Tuất', 'Hợi'];
  
  return {
    day: lunarDay,
    month: lunarMonth,
    year: lunarYear,
    isLeapMonth: isLeapMonth,
    lunarYearCanChi: `${CAN[canIndex]} ${CHI[chiIndex]}`,
  };
}

/**
 * Lấy Can Chi của năm
 */
export function getCanChiYear(year: number): { can: number; chi: number } {
  return {
    can: (year - 4) % 10,
    chi: (year - 4) % 12,
  };
}

/**
 * Lấy Can Chi của tháng âm lịch
 */
export function getCanMonth(yearCan: number, lunarMonth: number): number {
  // Công thức: Can tháng = (Can năm * 2 + tháng) % 10
  const startCan = (yearCan * 2 + 2) % 10;
  return (startCan + lunarMonth - 1) % 10;
}
